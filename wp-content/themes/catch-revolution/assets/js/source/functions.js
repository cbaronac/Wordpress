/* global catchRevolutionOptions */
 /*
 * Custom scripts
 * Description: Custom scripts for catchRevolution
 */
...
( function( $ ) { 
	jQuery( document ).ready( function() {
		...
		jQuery( window )
			.on( 'load.catchRevolution resize.catchRevolution', function() {
			...
			jQuery('#primary-search-wrapper .menu-inside-wrapper').on('focusout', function () {
			    ...
			    setTimeout(function () {
			        if ( ! $elem.find(':focus').length ) {
			        	jQuery( '#primary-search-wrapper #social-search-toggle' ).trigger('click');
			        }
			    }, 0);
			});
		} );
	...
	if ( $.isFunction( $.fn.owlCarousel ) ) {
		// Featured Slider
		var sliderOptions = {
			rtl:catchRevolutionOptions.rtl ? true : false,
			autoHeight:true,
			margin: 0,
			items: 1,
			nav: false,
		...
		var testimonialOptions = {
			...
			navText: [catchRevolutionOptions.iconNavPrev,catchRevolutionOptions.iconNavNext],
	...
	$( function() {
		...
		$( window ).on( 'load.abletone resize.abletone', function () {
	...
	function initMainNavigation( container ) {
		...
		var dropdownToggle = $( '<button />', { 'class': 'dropdown-toggle', 'aria-expanded': false })
			.append( $( '<span />', { 'class': 'screen-reader-text', text: catchRevolutionOptions.screenReaderText.expand }) );
		...
		container.find( '.dropdown-toggle' ).click( function( e ) {
			...
			// jscs:enable
			screenReaderSpan.text( screenReaderSpan.text() === catchRevolutionOptions.screenReaderText.expand ? catchRevolutionOptions.screenReaderText.collapse : catchRevolutionOptions.screenReaderText.expand );
		} );
	}
